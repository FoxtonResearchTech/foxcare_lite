...
class TestOP extends StatefulWidget {
  final String doctorName;
  const TestOP({super.key, required this.doctorName});

  @override
  State<TestOP> createState() => _TestOP();
}

class _TestOP extends State<TestOP> {
  ...
  void initState() {
    ...
    listenToAllOpTickets();

    // _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
    //   fetchData();
    // });
  }
  Map<String, StreamSubscription<QuerySnapshot>> _opTicketsListeners = {};



  void listenToAllOpTickets() {
    FirebaseFirestore.instance
        .collection('patients')
        .snapshots()
        .listen((patientsSnapshot) {
      for (var patientDoc in patientsSnapshot.docs) {
        final patientId = patientDoc.id;

        // Avoid duplicating listeners
        if (_opTicketsListeners.containsKey(patientId)) continue;

        final subListener = FirebaseFirestore.instance
            .collection('patients')
            .doc(patientId)
            .collection('opTickets')
            .snapshots()
            .listen((opTicketsSnapshot) {
          // Handle opTickets changes
          fetchData(); // Or refactor to only update what changed
        });

        _opTicketsListeners[patientId] = subListener;
      }
    });
  }
  ...
  void dispose() {
    _timer?.cancel();
    for (var sub in _opTicketsListeners.values) {
      sub.cancel();
    }
    _opTicketsListeners.clear();
    super.dispose();
  }
